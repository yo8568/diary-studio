...
把影片放進 Downloads（AirDrop 的預設位置）、Movies 或 Desktop，然後：

- 在 app 裡按右上角「換影片」，從縮圖網格選；清單依修改時間新到舊，剛傳進來的在第一個
- 或直接把影片拖到 app 圖示上，也可以用「打開檔案的應用程式」選 Diary Studio

選好之後**不需要再按任何按鈕**：轉檔、辨識語音、分辨說話者會自己接著跑，
頂端顯示目前階段、百分比與經過時間。

app 啟動時會回到上次的專案，因為校對通常分幾次做完；要開新的就按「換影片」。
同一支影片重開會沿用既有專案，不會重跑。
